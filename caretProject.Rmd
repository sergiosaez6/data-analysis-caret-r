...
$$
PC_1 = (0.4177783 \times median) + (0.2830979 \times Q75) - (0.3262841 \times IQR) -
$$
$$
(1834230 \times kurt) - (0.3135638 \times sp.ent) + (0.3800822 \times mode) +
$$
$$
(0.3178615 \times meanfun) + (0.2749328 \times minfun) + (0.2029431 \times maxfun) +
$$
$$
(0.3338909 \times meandom) + (0.1238045 \times mindom) - (0.1334400 \times modindx)
$$

$$
PC_2 = (-0.0814136658 \times median) - (0.3476275299 \times Q75) - (0.3753267077  \times IQR) -
$$
$$
(-0.1807885156 \times kurt) - (0.1891926361 \times sp.ent) - (0.0151021143 \times mode) +
$$
$$
(0.1387576565 \times meanfun) + (0.0007528243 \times minfun) - (0.4348259478 \times maxfun) -
$$
$$
(0.1064791249 \times meandom) + (0.4980567051 \times mndom) + (0.4418418481 \times modindx)
$$
...
En este caso se tiene un valor óptimo de **$accuracy=0.9709640$** para **$C=1$** y **$\sigma=1.123122$**, algo inferior al obtenido con el anterior modelo, pero con muchas menos variables (recordemos que pasamos de 12 a solamente 2).
\newpage
...
ggplot(knnBase)
```
El resultado obtenido es de **$accuracy=0.9697106$** para una **$k=7$**, semejante al peor de los casos con SVM-RBF.
...
ggplot(knnPCA)
```
Finalmente, con PCA se obtiene una **$accuracy=0.9579228$** durante la validación para una **$k=7$**.
...
\newpage
```{r, cache=TRUE}
diffs<-diff(resamps2, adjustment="bonferroni", test=t.test, confLevel=0.95)
summary(diffs)
```

Como se puede observar, el *$p-value=0.006755$* para la diferencia de medias de *accuracy*, un orden de magnitud menor que el $p-value=0.05$, por lo que se puede considerar que ambos clasificadores son significativamente diferentes desde el punto de vista estadístico (la hipótesis nula es que la diferencia de las medias era 0, es decir, que eran semejantes).
